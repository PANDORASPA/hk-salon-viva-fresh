...
export default function TimeOffEditor({ staffId }) {
  ...
  const load = async () => { setLoading(true); setError(''); try { const response = await fetch(`/api/admin/staff/${staffId}/time-off`); const data = await response.json(); if (!response.ok) throw new Error(data.error || '未能載入休假。'); setRows(data.timeOff || []) } catch (cause) { setError(cause.message) } finally { setLoading(false) } }
  useEffect(() => { load() }, [staffId])
  const create = async event => { event.preventDefault(); setError(''); const form = Object.fromEntries(new FormData(event.currentTarget)); try { const response = await fetch(`/api/admin/staff/${staffId}/time-off`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ startsAt: iso(form.startsAt), endsAt: iso(form.endsAt), reason: form.reason }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || '未能新增休假。'); event.currentTarget.reset(); load() } catch (cause) { setError(cause.message) } }
