...
const addDays = (day, days) => { const next = new Date(`${day}T00:00:00+08:00`); next.setUTCDate(next.getUTCDate() + days); return hkDay(next) }
...
export default function BookingCalendar() {
  ...
  const save = async (event, method) => {
    ...
    try {
      ...
      event.currentTarget.reset(); setShowCreate(false); load()
  ...
  return (
    <section className="admin-module" aria-labelledby="calendar-title">
      ...
      <div className="admin-calendar-controls">
        ...
        <button className="admin-action" onClick={() => setDay(current => addDays(current, mode === 'week' ? -7 : -1))}>上一個</button><label>日期<input type="date" value={day} onChange={event => setDay(event.target.value)} /></label><button className="admin-action" onClick={() => setDay(current => addDays(current, mode === 'week' ? 7 : 1))}>下一個</button>
      ...
      {loading ? <p aria-busy="true">正在載入預約…</p> : rows.length ? <div className="admin-calendar-table"><table><caption>{mode === 'week' ? `${day} 至 ${endDay}` : day}</caption><thead><tr><th>時間</th><th>客戶</th><th>服務</th><th>員工</th><th>狀態</th><th>操作</th></tr></thead><tbody>{rows.map(row => <tr key={row.id}><td>{label(row.startsAt)}</td><td>{row.customerName}<small>{row.customerPhone}</small></td><td>{row.serviceName}</td><td>{row.staffName}</td><td><span className={`status ${row.status}`}>{statuses.find(([value]) => value === row.status)?.[1] || row.status}</span></td><td><details><summary>改期</summary><form onSubmit={event => save(event, 'PATCH')}><input type="hidden" name="id" value={row.id} /><label>新時間<input name="startsAt" type="datetime-local" defaultValue={toInput(row.startsAt)} required /></label><select name="staffPreference" defaultValue={String(row.staffId || 'any')}><option value="any">自動安排</option>{staff.map(person => <option key={person.id} value={person.id}>{person.name}</option>)}</select><button className="admin-action">儲存改期</button></form></details></td></tr>)}</tbody></table></div> : <p className="admin-empty">這段時間沒有符合篩選的預約。</p>}
    </section>
  )
}
