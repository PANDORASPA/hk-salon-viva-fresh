...
export default function StaffHoursEditor({ staffId }) {
  ...
  const load = async () => {
    ...
    try { const response = await fetch(`/api/admin/staff/${staffId}/hours`); const data = await response.json(); if (!response.ok) throw new Error(data.error || '未能載入工時。'); setHours(data.hours?.length === 7 ? data.hours : defaults()) }
