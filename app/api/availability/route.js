...
export async function GET(request) {
  ...
  const [serviceRes, hoursRes, blockRes, appointmentsRes] = await Promise.all([
    db.from('services').select('id, name, duration_minutes').eq('id', serviceId).eq('published', true).maybeSingle(),
  ...
  const slots = buildAvailability({
    ...
    hours: hoursRes.data,
    blocked: Boolean(blockRes.data?.length),
