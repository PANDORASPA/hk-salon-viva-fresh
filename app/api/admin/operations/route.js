...
export async function GET() {
  ...
  const [appointments, packages, notifications, staff, services] = await Promise.all([
    context.db.from('appointments').select('id,status').gte('starts_at', hkMidnight(today)).lt('starts_at', tomorrow.toISOString()),
    context.db.from('customer_packages').select('id,expires_at,sessions_remaining,customers(name),packages(name)').gte('expires_at', tomorrow.toISOString()).lt('expires_at', expires.toISOString()).eq('is_active', true).order('expires_at').limit(20),
  ...
  if ([appointments, packages, notifications, staff, services].some(result => result.error)) return jsonError('Unable to load operations dashboard.', 500)
  const rows = appointments.data || []
  return NextResponse.json({
    today: { date: today, total: rows.length, pending: rows.filter(row => row.status === 'pending').length },
