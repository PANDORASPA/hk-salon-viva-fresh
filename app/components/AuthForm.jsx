...
export default function AuthForm({ mode = 'signin', admin = false }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      router.push(requested.startsWith('/') && !requested.startsWith('//') ? requested : (admin ? '/admin' : '/account'))
      router.refresh()
    } catch {
      setError('登入服務暫未能使用，請稍後再試。')
      setBusy(false)
    }
  }

  return (
    <form method="post" action="/api/auth/unavailable" className="salon-form auth-form" onSubmit={submit}>
      ...
      <button type="submit" className="salon-button" disabled={busy}>{busy ? 'Please wait…' : mode === 'signup' ? 'Create account' : 'Sign in'}</button>
