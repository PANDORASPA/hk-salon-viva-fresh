...
export function AppointmentsModule(){const r=useResource('/api/admin/appointments','appointments'),update=async(row,status)=>{await api('/api/admin/appointments',{method:'PATCH',body:JSON.stringify({id:row.id,status,adminNotes:row.admin_notes||''})});r.load()};return <Module title="Appointments" intro="Confirm, complete, cancel or mark client appointments as no-show."><State {...r}><div className="admin-list">{r.rows.map(row=><article key={row.id}><div><strong>{row.reference} · {row.customer_name}</strong><p>{new Date(row.starts_at).toLocaleString('en-GB',{timeZone:'Europe/London'})} · {row.services?.name}</p><p>{row.customer_phone} {row.customer_email||''}</p></div><div><span className={`status ${row.status}`}>{row.status}</span>{['confirmed','completed','cancelled','no_show'].map(status=><Button key={status} onClick={()=>update(row,status)}>{status}</Button>)}</div></article>)}</div></State></Module>}
