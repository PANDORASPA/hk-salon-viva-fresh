...
test('admin schedule change removes a staff member from public availability', async ({ page }) => {
  ...
  const startsAt = page.getByLabel(`${weekday} 開始`)
  const endsAt = page.getByLabel(`${weekday} 結束`)
  await expect(workday).toBeChecked()
  const original = { working: await workday.isChecked(), startsAt: await startsAt.inputValue(), endsAt: await endsAt.inputValue() }
  try {
    ...
    await page.getByRole('button', { name: /員工及排班/ }).click()
    await page.getByRole('button', { name: `${namespace} 員工 A` }).click()
    const restoredWorkday = page.getByLabel(`${weekday} 上班`)
    if (original.working) await restoredWorkday.check(); else await restoredWorkday.uncheck()
    await page.getByLabel(`${weekday} 開始`).fill(original.startsAt)
    await page.getByLabel(`${weekday} 結束`).fill(original.endsAt)
    await page.getByRole('button', { name: '儲存工時' }).click()
    await expect(page.getByText('已儲存每週工時。')).toBeVisible()
    await expect(restoredWorkday).toBeChecked({ checked: original.working })
  }
})
