...
test('authenticated package booking is refunded after customer cancellation', async ({ page }) => {
  ...
  const reference = await page.locator('article').filter({ hasText: '預約編號' }).locator('p').textContent()
  expect(reference).toBeTruthy()
  await page.goto('/account')
  const booking = page.getByRole('article', { name: `預約 ${reference}` })
  await expect(booking).toContainText(serviceName)
  await expect(booking).toContainText(`套票：${namespace} 套票 · 已扣減 · 剩餘 1/2 次`)
  page.once('dialog', (dialog) => dialog.accept())
  await booking.getByRole('button', { name: '取消' }).click()
  await expect(page.getByRole('status')).toContainText('套票次數已退還')
  await expect(booking).toContainText(`套票：${namespace} 套票 · 已退還 · 剩餘 2/2 次`)
