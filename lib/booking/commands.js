...
const DATABASE_CODES = Object.freeze({
  ...
  B0007: 'package_not_usable', B0008: 'booking_not_found', B0009: 'booking_not_changeable', B0018: 'slot_unavailable',
...
export async function createAppointment(db, input) {
  ...
  const appointment = await command(db, input?.audited ? 'admin_create_appointment_audited' : 'create_appointment_v2', {
    ...
    ...(!input?.audited ? { p_source: source } : {}), p_confirmation_token_hash: createHash('sha256').update(confirmationToken).digest('hex'),
...
export async function rescheduleAppointment(db, input) {
  ...
  return command(db, input?.audited ? 'admin_reschedule_appointment_audited' : 'reschedule_appointment_v2', {
    ...(input?.audited ? { p_actor_id: input.actorUserId } : {}),
...
export async function cancelAdminAppointment(db, input) {
  if (!input?.actorUserId) throw new BookingCommandError('authentication_required')
  return command(db, 'admin_cancel_appointment_audited', { p_actor_id: input.actorUserId, p_appointment_id: positiveBookingId(input.appointmentId) })
}

export async function setAdminAppointmentStatus(db, input) {
  if (!input?.actorUserId) throw new BookingCommandError('authentication_required')
  if (!['confirmed', 'completed', 'no_show'].includes(input.status)) throw new BookingCommandError('validation_error')
  const notes = input.adminNotes == null ? '' : String(input.adminNotes)
  if (notes.length > 2000) throw new BookingCommandError('validation_error')
  return command(db, 'admin_set_appointment_status_audited', { p_actor_id: input.actorUserId, p_appointment_id: positiveBookingId(input.appointmentId), p_status: input.status, p_admin_notes: notes })
}
