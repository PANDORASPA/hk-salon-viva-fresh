...
export function buildStaffAvailability({
  ...
  const rawBuffer = number(settings.booking_buffer_minutes ?? settings.bufferMinutes ?? settings.buffer_minutes, 15)
  const buffer = Number.isInteger(rawBuffer) && rawBuffer >= 0 && rawBuffer <= 120 ? rawBuffer : 15
