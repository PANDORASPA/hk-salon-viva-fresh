...
function serves(staff, service) {
  ...
  if (values == null && staff?.service_id == null) return true
...
export function buildStaffAvailability({
  ...
  if (weekday == null || isBlocked(blocked, dateText) || !shop || duration <= 0 || buffer < 0 || step <= 0) return result
  ...
  for (const person of eligible) {
    ...
    for (let minute = start; minute + duration + buffer <= end; minute += step) {
      ...
      if (Number.isFinite(current) && (candidateStart < current + lead * MINUTE || candidateStart > current + horizon * 24 * 60 * MINUTE)) continue
