...
function endsAfter(query, column, value) {
  return typeof query.gt === 'function' ? query.gt(column, value) : query.gte(column, value)
}
...
export async function loadAvailability({ db, date, serviceId, logger = console }) {
  ...
  const results = await Promise.all([
    ...
    endsAfter(db.from('staff_time_off')
      ...
      .lt('starts_at', window.end), 'ends_at', window.start),
    endsAfter(activeAppointments(db.from('appointments')
      .select('staff_id,starts_at,occupied_until,status'))
      .lt('starts_at', window.end), 'occupied_until', window.start),
