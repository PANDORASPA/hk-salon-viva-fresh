...
function normaliseKey(key, value) {
  if (NUMERIC_KEYS.has(key)) {
    ...
    const ranges = { booking_buffer_minutes: [0, 120], slot_step_minutes: [5, 120], minimum_lead_minutes: [0, 10080], maximum_advance_days: [1, 730], reminder_hours_before: [1, 168], cancel_cutoff_hours: [0, 168] }
    const [min, max] = ranges[key] || [0, Number.MAX_SAFE_INTEGER]
    return Number.isSafeInteger(n) && n >= min && n <= max && (key !== 'slot_step_minutes' || 60 % n === 0) ? n : DEFAULTS[key]
  }
...
/** Strict mutation boundary; permissive normalisation is only for legacy reads. */
export function validateSettings(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).some(key => !Object.hasOwn(DEFAULTS, key))) throw new Error('設定格式不正確。')
  for (const [key, value] of Object.entries(input)) {
    if (NUMERIC_KEYS.has(key) && (typeof value !== 'number' || !Number.isSafeInteger(value) || normaliseKey(key, value) !== value)) throw new Error('請輸入有效的預約規則；服務緩衝最多 120 分鐘。')
    if (BOOLEAN_KEYS.has(key) && typeof value !== 'boolean') throw new Error('通知設定必須為開啟或關閉。')
    if (STRING_KEYS.has(key) && value !== DEFAULTS[key]) throw new Error('此通知供應商尚未支援。')
  }
  return { ...input }
}
