  ...
  <table role="presentation" class="container" width="600" align="center" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:8px;overflow:hidden">
    ...
    <tr>
      <td class="footer" style="padding:16px 32px 24px;background:#faf7f1;color:#706961;font-size:12px;text-align:center;border-top:1px solid #ded5c8">
        ...
        <p style="margin:0">如有問題，請使用網站上已提供的聯絡方式。</p>
      </td>
    </tr>
  </table>
</body>
 ...
 *   { ok: true, status: 'sent', id }              after provider acceptance
 *   { ok: true, status: 'dry_run' }               when delivery is simulated
 *   { ok: false, status: 'failed', reason }       on failure
 *
...
export async function sendEmail({ to, subject, html, text, preheader }) {
  if (!to) return { ok: false, status: 'failed', reason: 'no_recipient' }
  if (!subject || (!html && !text)) {
    return { ok: false, status: 'failed', reason: 'missing_content' }
  }
  ...
  try {
    ...
    const { from } = getEnv()
    if (!from) return { ok: false, status: 'failed', reason: 'sender_not_configured' }
    ...
    if (result.error) {
      return { ok: false, status: 'failed', reason: sanitizeProviderReason(result.error.message) }
    }
    return { ok: true, status: 'sent', id: result.data?.id || null }
  } catch (err) {
    return { ok: false, status: 'failed', reason: sanitizeProviderReason(err?.message) }
  }
}

function sanitizeProviderReason(value) {
  const message = String(value || '').replace(/[\r\n]+/g, ' ').replace(/\b(api[_ -]?key|authorization|bearer)\s+[^\s]+/ig, '$1 [redacted]').trim()
  return message ? `provider_error: ${message.slice(0, 180)}` : 'provider_error'
}
...
export const __testing = { isEmailLiveMode, getEnv, escapeHtml, stripHtml, renderBrandedEmail, sanitizeProviderReason }
