...
export async function sendBookingNotification({ event, booking, service, prevStartsAt, packageRefunded = false }) {
  ...
  const consoleEnabled = DRY_RUN || settings.notify_console_enabled
  ...
  const record = {
    ...
    channel_results: { supabase: { ok: false, mode: 'persistence_pending', reason: 'channel_results_pending' } },
  }
  const notificationLog = await logToSupabase(record)
  if (notificationLog) channels.push('supabase')
  results.supabase = { ok: Boolean(notificationLog), reason: notificationLog ? undefined : 'notification_log_failed' }
  ...
  if (consoleEnabled) {
    ...
    results.console = { ok: false, mode: 'disabled', reason: 'console_disabled' }
  }

  // 2) whatsapp (stub)
  if (!whatsappEnabled) {
    results.whatsapp = { ok: false, mode: 'disabled', reason: 'whatsapp channel disabled in settings' }
  } else if (DRY_RUN || process.env.NOTIFY_WHATSAPP_PROVIDER === 'off' || !process.env.NOTIFY_WHATSAPP_PROVIDER) {
  ...
  if (!emailEnabled) {
    results.email = { ok: false, mode: 'disabled', reason: 'email channel disabled in settings' }
  ...
  if (notificationLog) {
    try {
      ...
      results.supabase = { ok: false, reason: 'outcome_persist_failed' }
    }
  }
  return { ok: outcomePersisted, channels, results, outcomePersisted }
}

export const __testing = { render, formatHkTime, TEMPLATES, setServiceClient: client => { injectedServiceClient = client } }
