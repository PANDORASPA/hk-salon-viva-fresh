...
export async function sendBookingNotification({ event, booking, service, prevStartsAt, packageRefunded = false }) {
  ...
  // 4) email (stub)
  if (DRY_RUN || process.env.NOTIFY_EMAIL_PROVIDER === 'off' || !process.env.NOTIFY_EMAIL_PROVIDER) {
    results.email = { ok: false, mode: 'dry_run', reason: 'no provider configured' }
  } else {
    // TODO: implement Resend / Postmark / SES call here
    results.email = { ok: false, mode: 'stub', reason: 'not yet implemented' }
  }
