...
function reminderCopy(appointment, reminderHours, cancelCutoffHours) {
  const serviceName = appointment.services?.name || appointment.service_name || '服務'
...
export async function dispatchReminders({ db, settings, now = new Date(), sendEmail }) {
  ...
  const { error: reconciliationError } = await db.rpc('reconcile_expired_ambiguous_reminders', { p_now: now.toISOString(), p_limit: 50 })
  if (reconciliationError) throw reconciliationError

  // The scheduled hour finds new reminders. A small, database-bounded retry
  // query also revisits prior failed or expired claims that are still for an
  // eligible future appointment in this configured reminder horizon.
  const { data: retryable, error: retryError } = await db.rpc('find_retryable_reminder_appointments', {
    p_reminder_window_hours: reminderHours,
    p_now: now.toISOString(),
    p_limit: 50,
  })
  if (retryError) throw retryError
  const appointments = [...new Map([...(upcoming || []), ...(retryable || [])].map(appointment => [appointment.id, appointment])).values()]

  const results = { checked: appointments.length, sent: 0, dry_run: 0, skipped: 0, failed: 0, items: [] }
