...
async function persistOutcome(db, id, channelResults) {
  const { error } = await db.from('notifications').update({ channel_results: channelResults }).eq('id', id)
  return !error
}
...
export async function dispatchReminders({ db, settings, now = new Date(), sendEmail }) {
  ...
  for (const appointment of upcoming || []) {
    ...
    const outcome = appointment.customer_email ? await sendEmail({ to: appointment.customer_email, subject: copy.subject, text: copy.text }) : emailResult('failed', 'no_recipient')
    if (!await persistOutcome(db, claim.notification_id, { email: outcome })) { results.failed += 1; results.items.push({ id: appointment.id, status: 'persistence_failed', reason: 'outcome_persist_failed' }); continue }
