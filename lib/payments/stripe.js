/**
 * Stripe wrapper. Purchases are unavailable until real Stripe credentials are configured.
 *
...
export function isStripeConfigured() {
  const env = getEnv()
  return Boolean(env.secretKey && env.webhookSecret && env.publishableKey)
}

export async function getStripe() {
  if (stripeClient) return stripeClient
  if (!isStripeConfigured()) throw new Error('網上付款尚未啟用，請聯絡店舖安排購買。')
  const env = getEnv()
...
export async function verifyWebhook({ rawBody, signature, sessionId }) {
  if (!isStripeConfigured()) throw new Error('網上付款尚未啟用，不能驗證付款通知。')
  const stripe = await getStripe()
  const env = getEnv()
