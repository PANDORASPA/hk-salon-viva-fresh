...
| 電郵通知（啟用時） | `RESEND_API_KEY`、`NOTIFY_EMAIL_FROM`、`NOTIFY_EMAIL_PROVIDER`、`NOTIFY_DRY_RUN` |
...
`CRON_SECRET` 透過 `Authorization: Bearer` 驗證；舊有 query-secret 相容路徑仍存在，但新 cron 設定應使用 Authorization header。沒有 Resend 或 Stripe 正式設定時，先在後台確認通知／購買功能如實顯示為停用或 dry-run，不能把它們當作已送出或已付款。
